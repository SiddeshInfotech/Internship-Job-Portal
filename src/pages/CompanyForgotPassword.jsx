...
function CompanyForgotPassword() {
  ...
  const handleSendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await clientAxios.post('/client/forgot-password', { email });
      if (res.data.dev_otp) setDevOtp(res.data.dev_otp);
      setMessage(res.data.message || 'A 6-digit OTP code has been sent to your company email.');
      setStep(2);
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      await clientAxios.post('/client/verify-reset-otp', { email, otp });
  ...
  return (
    <>
      ...
      <div className="min-h-screen relative flex flex-col items-center justify-center bg-[#FAFAFA] font-sans px-4 sm:px-6 overflow-hidden">
        <div className="absolute top-[-15%] left-[-10%] w-[500px] h-[500px] rounded-full bg-blue-200/30 blur-3xl pointer-events-none -z-10" />
        <div className="absolute bottom-[-15%] right-[-10%] w-[600px] h-[600px] rounded-full bg-indigo-200/20 blur-3xl pointer-events-none -z-10" />
            ...
            {step === 1 && (
              <>
                <div className="text-center mb-8">
                  ...
                  <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Company Password Recovery</h2>
                  <p className="text-sm font-medium text-slate-500 mt-2 leading-relaxed">
                    Enter your company email address and we'll send a 6-digit OTP code to reset your password.
                  </p>
                </div>
            ...
            {step === 4 && (
              <div className="text-center py-4 animate-success">
                ...
                <p className="text-sm font-medium text-slate-500 leading-relaxed mb-8">
                  Your company account password has been successfully updated. You can now sign in.
                </p>
                <Link 
                  to="/company/login" 
